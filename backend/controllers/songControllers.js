...
export const createAlbum = TryCatch(async (req, res) => {
  ...
  const cloud = await cloudinary.v2.uploader.upload(fileUrl.content,{folder: "Spotify/Albums"});
...
export const addThumbnail = TryCatch(async (req, res) => {
  ...
  const cloud = await cloudinary.v2.uploader.upload(fileUrl.content, {folder: "Spotify/Thumbnails"});
